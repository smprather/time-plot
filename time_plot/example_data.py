...
class SpicePwlSourceSpec(NamedTuple):
    name: str
    node: str
    vlow: float
    vhigh: float
...
def write_vcd_example(output_path: Path) -> Path:
    """Generate a small scalar-logic VCD with three signals."""
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(
        """$date
  Generated by time-plot example data.
$end
$version
  time-plot
$end
...
def write_example_data_files(output_dir: Path) -> list[Path]:
    ...
    written.append(write_vcd_example(output_dir / "logic.vcd"))
    return written
